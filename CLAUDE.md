...
- **"Recursos del proceso de selección"** — a single-card grid (`.cards.single`) containing a `.resource-list` of `.resource-item` rows: handwritten test answers (`Recursos/Scanner.pdf`), presentation video, AI-skills video, and the case videos for Growth Autopilot and Lifecycle Autopilot, each `.resource-desc` stating which project it corresponds to. The "Video de habilidades IA" row and the Cotizador IA case video are the same file — see the `Recursos/Videos` note above. This is the single place all videos are linked from — see that note also for why they aren't duplicated inside each project's own page. Each row either links out via `.resource-link` (a local path or a Vercel Blob URL) or shows a `.resource-pending` badge ("Próximamente") when the underlying file isn't wired up yet.
- **"Proyecto real de cliente"** — a single-card grid (`.cards.single`) for the Cotizador IA project, styled with the amber accent (`.card-tag.amber`) to visually distinguish a real delivered project from the Alegra proposals. Only a "Ver PDF" button (`.card-actions.full`) since no interactive HTML exists for it — its case video lives in the Recursos section instead, not on this card.
