...
- **"Recursos del proceso de selección"** — a single-card grid (`.cards.single`) containing a `.resource-list` of `.resource-item` rows: handwritten test answers (`Recursos/Scanner.pdf`), presentation video, AI-skills video (still pending — no file yet), and the three per-project case videos (Growth Autopilot, Lifecycle Autopilot, Cotizador IA "Caso Real"), each `.resource-desc` stating which project it corresponds to. This is the single place all videos are linked from — see the `Recursos/Videos` note above for why they aren't duplicated inside each project's own page. Each row either links out via `.resource-link` (a local path or a Vercel Blob URL) or shows a `.resource-pending` badge ("Próximamente") when the underlying file isn't wired up yet.
